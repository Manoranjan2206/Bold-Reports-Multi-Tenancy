  ...
  const groupedUsers = useMemo(() => {
    ...
    mockData.forEach((record: SalesRecord) => {
        ...
        if (!uniqueUserMap.has(key)) {
            const tenantInfo = getTenantById(record.TenantId);
            const email = `${record.UserName.toLowerCase().replace(' ', '.')}@${tenantInfo.domain}`;
            const dbMapping = `'sales_analysis_db':'${tenantInfo.slug}_sales_analysis'`;
